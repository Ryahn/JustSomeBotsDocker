...
RUN echo "Adding ppa:mc3man/trusty-media" && add-apt-repository -y -r ppa:mc3man/trusty-media
RUN rm -f /etc/apt/sources.list.d/mc3man-ubuntu-trusty-media-xenial.list
RUN echo "Adding ppa:chris-lea/libsodium" && add-apt-repository -y -r ppa:chris-lea/libsodium
